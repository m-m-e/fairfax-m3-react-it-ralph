import React from 'react';
import Card from './card/Card';



class App extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
        name:'',
        job:'',
        email:'',
        tel:'',
        linkedin:'',
        github:'',
        palette: 1,
        hidden: 'true'
    }
    this.handleInput = this.handleInput.bind(this);
    this.handleColor = this.handleColor.bind(this);
}
handleHidden(event) {
  const whichCollapse = event.currentTarget;
  console.log(whichCollapse);
  this.setState  ({
    hidden: whichCollapse,
  })
}

handleColor(event){
    const value = parseInt(event.currentTarget.value);
    this.setState({palette: value});
}
handleInput(event) {
    const currentField = event.currentTarget;
    const key = currentField.id;
    const value = currentField.value;
    const newValue = {};
    newValue[key] = value;

    console.log(newValue);
    this.setState(newValue);
}
  render() {

    const url = 'data:image/png/jpg/svg/jpeg;base64,';
    
    return <Card url={url} state={this.state} handleColor={this.handleColor} handleInput={this.handleInput} actionToHidden={this.handleHidden}/>;    
  }
}

export default App;
